...
#### 方式 A：Python 像素级分析（首选，最精确）⭐

当模型不支持多模态（如 DeepSeek V4/V3 等纯文本模型）或追求更高精度时，
使用 `scripts/skin_analyze.py` 或手动执行 Pillow + NumPy 像素级肤色分析。

**核心指标与阈值（经验值）：**

| 指标 | 暖调 | 微暖 | 中性 | 冷调 |
|------|------|------|------|------|
| R/B 比值 | >1.12 | 1.03-1.12 | 0.98-1.03 | <0.98 |
| 饱和度 | >0.22 鲜艳 | 0.14-0.22 中等 | 0.08-0.14 柔和 | - |
| 明度 | >180 浅色 | 150-180 中偏浅 | 120-150 中等 | - |
| 对比度 | 眉毛vs额头亮度差 >0.3 高, 0.15-0.3 中, <0.15 低 ||

**采样策略：** 脸颊区域（避开高光/阴影），多点多区域取均值。

#### 方式 B：vision_analyze（备选，当模型支持多模态时）
